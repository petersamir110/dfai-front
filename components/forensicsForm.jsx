  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/backend/memory/path", {
        ...
        body: JSON.stringify({ project_name: projectName, file_path: finalPath }),
      });
    ...
    <div className="flex flex-col items-center">
      <form className="w-120 flex flex-col gap-6" onSubmit={handleSubmit}>
        ...
        <Field className="flex flex-col gap-2">
          <span className="text-xs font-mono uppercase tracking-widest text-slate-400 self-start">Project Name</span>
          <Input 
        ...
        <Field className="flex flex-col gap-2">
          <span className="text-xs font-mono uppercase tracking-widest text-slate-400 self-start">Dump File Path</span>
        ...
        <div className="w-full max-w-md mx-auto mt-4 mb-2 select-none relative">
          ...
          <div className="flex justify-between items-center relative z-10">
            ...
            {/* الخطوة 2: Extracted */}
            <div className="flex flex-col items-center flex-1">
              <div className={`size-7 rounded-full flex items-center justify-center border transition-all duration-300 ${
                percent === 100
                ...
                {percent === 100 ? (
                  ...
                  <div className={`size-1.5 rounded-full transition-all duration-300 ${
                    activeStep === 2 ? "bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.6)]" : "bg-slate-700"
                  }`} />
                )}
              </div>
              <span className={`text-[11px] font-mono mt-2 tracking-wide transition-colors duration-300 ${
                percent === 100 ? "text-emerald-400 font-semibold" : activeStep === 2 ? "text-slate-300 font-semibold" : "text-slate-600"
              }`}>
                Extracted
              </span>
            </div>
        ...
        <div className="w-full mt-2 flex flex-col gap-2 select-none">
          <div className="flex justify-between items-center text-[11px] font-mono tracking-wider">
            ...
            <span className="text-slate-400">
              Tasks: <span className={`font-bold ${percent === 100 ? "text-emerald-500" : "text-red-500"}`}>{taskCount}</span> 
              <span className="text-slate-600 mx-2">|</span> 
              <span className={`font-bold ${percent === 100 ? "text-emerald-500" : "text-slate-300"}`}>{percent}%</span>
            </span>
          </div>
